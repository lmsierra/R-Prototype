...
//-----------------------------------------------------------------------------
void FONT_DrawChar(vec2 p0, vec2 p1, unsigned char ch, vec3 color)
{
...
//-----------------------------------------------------------------------------
void FONT_DrawString(vec2 p0, const char string[], vec3 color, vec2 size)
{
  int n = (int)strlen(string);
  for (int i = 0; i < n; i++)
  {
    FONT_DrawChar(p0, vadd(p0, size), string[i], color);
    p0 = vadd(p0, vmake(size.x, 0.f));
  }
}
